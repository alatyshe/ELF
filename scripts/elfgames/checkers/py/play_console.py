...
def main():
  ...
  # Also init GameContext from C++ library wrapped by GC from python side
  ...
  # We registered their names earlier when the game was initialized
  # (names were registered on the python and C++ sides),
  # Now its a registration of methods that will be called
  # when we try to pass batch on eval from C++ to Python
  # Example:
  # We register "human_actor" as key and register the same method 
  # on python side. 
  # When our AIClientT calls method act(it takes 2 parameters, state and key)
  # act connect to python and transmits the state by key(human_actor, checkers_actor_black)
  # to this methods.
