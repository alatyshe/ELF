...
class MCTSActor {
 public:
  ...
  MCTSActor(elf::GameClient* client, const MCTSActorParams& params)
    ...
    display_debug_info("MCTSActor", __FUNCTION__, RED_B);
...
class MCTSGoAI : public elf::ai::tree_search::MCTSAI_T<MCTSActor> {
 public:
  MCTSGoAI(
    ...
    display_debug_info("MCTSGoAI", __FUNCTION__, RED_B);
  }
...
class CheckersMCTSActor {
 public:
  ...
  CheckersMCTSActor(elf::GameClient* client, const MCTSActorParams& params)
      : params_(params),
        rng_(params.seed),
        logger_(elf::logging::getLogger("elfgames::go::mcts::CheckersMCTSActor-", "")) {
 ...
 private:
  ...
  // void remove_pass_if_dangerous(const CheckersState& s, bool* pass_enabled) {
  //   display_debug_info("CheckersMCTSActor", __FUNCTION__, RED_B);
